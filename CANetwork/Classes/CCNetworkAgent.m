...
@implementation CCNetworkAgent
{
    ...
    NSMutableDictionary<NSNumber *, CCBaseRequest *> *_requestsRecord;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _processingQueue = dispatch_queue_create("com.ash.networkagent.processing", DISPATCH_QUEUE_CONCURRENT);
...
- (AFJSONResponseSerializer *)jsonResponseSerializer {
    if (!_jsonResponseSerializer) {
        ...
        _jsonResponseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"text/plain", @"application/json", nil];
...
- (void)cancelAllRequests {
    ...
    if (allKeys && allKeys.count > 0) {
        ...
        for (NSNumber *key in copiedKeys) {
            Lock();
            CCBaseRequest *request = _requestsRecord[key];
            Unlock();
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    Lock();
    CCBaseRequest *request = _requestsRecord[@(task.taskIdentifier)];
    Unlock();
...
- (void)requestDidFailWithRequest:(CCBaseRequest *)request error:(NSError *)error {
    request.error = error;
    CCLog(@"Request %@ failed, status code = %ld, error = %@",
...
- (void)removeRequestFromRecord:(CCBaseRequest *)request {
    ...
    CCLog(@"Request queue size = %zd", [_requestsRecord count]);
    Unlock();
}
